...
def ICIC_RB_allocate(UE_list, BS:BS, RB_per_UE, serving_map:ServingMap):
    for _UE in UE_list:
        if _UE.posi_type == 'center':
            RB_arr = BS.resource_map.center_RB_sorted_idx[:RB_per_UE]
        elif _UE.posi_type == 'edge':
            RB_arr = BS.resource_map.edge_RB_sorted_idx[:RB_per_UE]
        else:
            RB_arr = BS.resource_map.RB_sorted_idx[:RB_per_UE]
        Nt_arr = np.array([])
