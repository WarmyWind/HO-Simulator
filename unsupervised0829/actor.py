...
class ActorNetwork:
    def __init__(self, learning_rate, num_hidden_1, num_hidden_2, num_hidden_3):
        ...
        self.actor_gradients = tf.gradients(self.action, self.actor_variables, -self.critic_gradients_action)  #-self.critic_gradients_action
    ...
    def _build_network(self, name):
        ...
        input_state = tf.compat.v1.placeholder(tf.float32, [None, 4])
        with tf.compat.v1.variable_scope(name):
            ...
            layer_3 = slim.layers.fully_connected(layer_2, self.num_hidden_3,
                                                        weights_initializer=self.weights_initializer)
            action = slim.layers.fully_connected(layer_3, 1, activation_fn=tf.nn.sigmoid,
                                                       weights_initializer=tf.random_uniform_initializer(-3e-3, 3e-3),
                                                       biases_initializer=tf.constant_initializer(-2))
