...
def start_simulation(PARAM, BS_list, UE_list, shadow, large_fading:LargeScaleFadingMap, small_fading:SmallScaleFadingMap,
    ...
    for drop_idx in range(1, SimConfig.nDrop):
        '''更新UE位置'''
        for _UE in UE_list:
            _UE.update_posi(UE_posi[_UE.type, drop_idx, _UE.type_no])
...
def init_all(PARAM, Macro_Posi, UE_posi, shadowFad_dB):
    ...
    if len(UE_posi.shape) == 2:
        for i in range(PARAM.nUE):
            UE_list.append(UE(i, i, UE_posi[0,i]))
    elif len(UE_posi.shape) == 3:
        for i in range(UE_posi.shape[0]):
            _UE_posi = UE_posi[i]
            for j in range(PARAM.nUE_per_type):
                UE_list.append(UE(i*PARAM.nUE_per_type+j, j, _UE_posi[0, j], i, True))
...
if __name__ == '__main__':
    ...
    Macro_Posi = road_cell_struct(PARAM.nCell, 250)
